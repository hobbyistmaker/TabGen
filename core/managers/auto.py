...
class Fingers:
    ...
    def __init__(self, app, ui, inputs):
        ...
        self.finger_length = self._get_finger_length(self._name, self.adjusted_length, self.fingers, self.kerf)
        ...
        self.start = self._get_start(self._name, self.offset, self.kerf, self.finger_length, self.tab_first)
    ...
    @staticmethod
    def _get_finger_length(alias, adjusted_length, fingers, kerf):
        return Property(alias('finger_length'), adjusted_length.value / fingers.value,
                        '({}/{})- {}'.format(adjusted_length.name, fingers.name, kerf.name),
    ...
    def _get_pattern_distance(alias, adjusted_length, finger_length, kerf, tab_first):
        if tab_first:
            ...
            expression = '(({} - {}*3)) - {}*3'.format(adjusted_length.name, finger_length.name, kerf.name)
        else:
            value = adjusted_length.value - finger_length.value
            expression = '(({} - {}))'.format(adjusted_length.name, finger_length.name, kerf.name)
    ...
    @staticmethod
    def _get_start(alias, offset, kerf, finger_length, tab_first):
        if tab_first:
            value = offset.value + finger_length.value + kerf.value/2
            expression = '({} + {} + {})'.format(offset.expression, finger_length.name, kerf.name)
        else:
            value = offset.value + kerf.value
            expression = '({} + {})'.format(offset.expression, kerf.name)
    ...
    def _get_offset(alias, margin, kerf, tab_first):
        if tab_first:
            ...
            value = margin.value
            expression = '({})'.format(margin.name, kerf.name)
    ...
    def save(self, properties):
        ...
        if left_dimension:
            ...
            properties.left_dimension.parameter.name = self.offset.name
        right_dimension = getattr(properties, 'right_dimension', None)
        if right_dimension:
            properties.left_dimension.parameter.expression = properties.left_dimension.parameter.name
