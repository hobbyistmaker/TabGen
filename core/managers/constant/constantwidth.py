...
class ConstantWidthFingers:
    ...
    def _get_adjusted_depth(alias, depth, kerf):
        return Property(alias('adjusted_depth'), -abs(depth.value) - abs(kerf.value),
                              '-(abs({}) - abs({}))'.format(depth.name, kerf.name),
    ...
    def _get_pattern_distance(alias, finger_distance, default_width, tab_first):
        if tab_first:
            value = finger_distance.value - default_width.value*3 - kerf.value
            expression = '(({} - {}*3))'.format(finger_distance.name, default_width.name, kerf.name)
        else:
    ...
    def _get_start(alias, offset, finger_length, kerf, tab_first):
        if tab_first:
            ...
            expression = '({} + {} + {}/2)'.format(offset.expression, finger_length.name, kerf.name)
        else:
            value = offset.value
